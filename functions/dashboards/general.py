...
def render(conn,contenedor_filtros):
    ...
    mask3 = df['F_Conclusion'].dt.year.between(rango_anios[0], rango_anios[1])
 
    df_f  = df [mask].copy()
    df2_f = df2[mask2].copy()
    salidas = df[mask3].copy()
 
    if subprocu_sel != 'Todas':
        df_f  = df_f [df_f ['SubProcu'] == subprocu_sel]
        df2_f = df2_f[df2_f['SubProcu'] == subprocu_sel]
        salidas = salidas[salidas['SubProcu']== subprocu_sel]
 
    # ── MÉTRICAS ───────────────────────────────
    total      = len(df_f['Expediente'].drop_duplicates())
    concluidos = salidas['F_Conclusion'].notna().sum()
    tasa       = f"{(concluidos/total*100):.1f}%" if total > 0 else "0%"
    tiempo_p   = salidas.loc[salidas['F_Conclusion'].notna(), 'TiempoDias'].mean()
    ...
    with c2:
        st.plotly_chart(fig_conclusiones(salidas), width='stretch')
...
def fig_insidencia(df2_f,columna):
    ...
    grupos['Variable_corta'] = grupos['Variable'].apply(
    ...
    fig.update_yaxes(
        tickmode='array',
        tickvals=grupos['Variable'],
        ticktext=grupos['Variable_corta']
    )
    fig.update_traces(
        hovertemplate='<b>%{y}</b><br>Insidencia: %{x}<extra></extra>'
    )
    return fig
